...
% \JH{ there is a single sample with probability of 1, so you just minimize the
% inside of the big bracket and ignore the rest.}
%
...
They propose using two types of hints from the teacher model. First, to bring
...
used.
% \JH{explain more clearly}

% ------------------------------------------------------------------------------
\paragraph{\Acl{jmnat}.} Following the practice from training the BERT model
...
The decoding process has three stages. First, the block of predictions is
...
%
%\JH{explain how inference works, begin with why is this iterative}
...
latency of 607 ms \JH{konkretne kdo}, which is the result of
...
(\acs{wmt}\glsunset{wmt}, \citealp{heafield-etal-2020-findings,
  heafield-etal-2021-findings}).\JH{update bib when ready, separate WMT from
  cite}
...
and throughput on a multi-core CPU. \JH{shrnout poznatky ze shared tasku}
