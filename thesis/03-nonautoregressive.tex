...
Very recently, a \ac{glat} model appeared amongst the top-scoring models at the
...
\JH{say something at least as vague as the name of this category}
...
\paragraph{DCRF} %\textbf{CRF decoding on top of vanilla NAT, not really NAT
                 %(similar to CTC+beam)}
\citet{sun2019fast} propose to use \ac{crf} instead of the argmax decoding from
the \ac{nat} model, which allows the modeling of causal relations within the
output sequence. Although this process is not \acl{ar} given the nature of the
...
\citep{ma-etal-2019-flowseq} -- NAR conditional sequence generator with
generative flow. The authors use an intermediate latent variable $z$, but they
argue that the prior distribution $p(z|x)$ needs to be complex to model all of
the dependency relations. They propose to use generative flows
\citep{rezende2015variational} for deriving the complex distribution from a
simple prior using a series of invertible transformations. \JH{megacomplex
  paper}
...
valid point, but on the other hand, it allows for doubts whether the difference
between large \ac{ar} and \ac{nar} model variants will be the same or not.

Another point is that even a Transformer base model can be trained in a better
way than as reported by \citet{vaswani2017attention} and as referenced by many
\ac{nat} papers as their baseline, achieving a better translation quality
...
100 ms with the same model under the same hardware and hyper-parameter settings
...
\JH{the step numbers in table \ref{tab:related:wmt14} are false and do not say
  much anyway.}
