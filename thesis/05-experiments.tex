...
\JH{We also see the same ranking of the NAR models, the larger student model,
  the better, with the exception of the Micro En-De model, which is better than
  the base model (this follows the situation during validation). In experiments
  with tied embeddings, the micro model scored in the expected position between
  the small and tiny models (todo: confirm this) }
...
\JH{Unlike our previous study in Chapter \ref{chap:nar-nmt-ctc}, we found
  checkpoint averaging does not have a significant effect on the translation
  quality (in terms of BLEU), and that the effect is even lower for fully
  converged models.}
...
\begin{table}
  ...
  \caption{The \acs{bleu} scores of the best models on the WMT~14 test set
    after 300k updates and at the end of the training. \JH{Add scores with
      checkpoint averaging, add one related work to compare.}}
...
albeit on a slightly different server architecture. \JH{TODO: For better
  comparison, we also report the GPU decoding times on other GPU types.}
