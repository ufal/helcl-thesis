...
Most commercial \gls{mt} models are available through a cloud-based service,
...
Using state-of-the-art translation models locally often requires specialized
hardware, such as one or more GPU cards. Otherwise, the time for translating a
single sentence can easily reach more than a second with unoptimized models
running on a standard CPU.

%lokální modely maj tu výhodu, že to běží u mně, takže to jde pouyžít offline a taky se nemusím bát o svoje soukromý data.

However, locally-deployed \gls{mt} models does provide a number of advantages.
First, the models can be used in places without internet connection. Second,
the data are not being sent to a 3rd party server, and therefore it is suitable
for translating private or confidential data.


% přehled real-world aplikací kde efficiency je důležitá
%Most
%\JH{Research projects like Bergamot (cite) try to bring the MT models to the
%  end-user, opening possibilities for  }
