...
\JH{už to řikám v \ref{sec:encdec}.}  Most of the \gls{nmt} architectures have
...
% text nejsou čísla, neuronky hrotěj čísla There is a number of problems that
...
% one-hot, simple In this section, we begin with representation of words. The
most straightforward, and also the most common way to represent words in neural
...
% embeddingy Using one-hot representation help us convert discreet words to
...
\emph{word embeddings} are used \citep{bengio2003neural} \JH{pridat vic
citaček}. The idea is to associate a real-valued vector with each word, so that
...
end-to-end along with the translation model.  % formalne embeddingy
\JH{odstavce?} Formally, the word embedding layer is defined as follows. For a
...
The \gls{bpe} algorithm %(Algorithm ~\ref{alg:learn-bpe}) proceeds as
...
\begin{eqnarray} % attn energies e_{ij} & = & v_a^\top \tanh (W_a s_{i-1} + U_a
h_j + b_a) + b', \label{eq:attn-energies} \\
%
% attn distro \alpha_{ij} & = & \frac{\exp(e_{ij})}{\sum_{k=1}^{T_x}
\exp(e_{ik})}, \\ %\softmax_{j \in 1, 2, \ldots, T_x } & = & e_{ij} \\
%
